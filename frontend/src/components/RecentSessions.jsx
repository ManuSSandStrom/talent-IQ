import { Code2, Clock, Users, Trophy, Loader, TrashIcon } from "lucide-react";
import { getDifficultyBadgeClass } from "../lib/utils";
import { formatDistanceToNow } from "date-fns";
import { useUser } from "@clerk/clerk-react";

function RecentSessions({ sessions, isLoading, onDeleteSession }) {
  const { user } = useUser();
  return (
            ...
            sessions.map((session) => (
              ...
              >
                {session.status === "active" && (
                 ...
                 {user?.id === session.host?.clerkId && (
                     <button 
                        className="absolute top-3 right-3 btn btn-ghost btn-xs text-error z-10"
                        style={{ right: session.status === "active" ? '5rem' : '0.75rem' }} 
