...
export const useAdmitGuest = () => {
  const result = useMutation({
    mutationKey: ["admitGuest"],
    mutationFn: sessionApi.admitSession,
    onSuccess: () => toast.success("Guest admitted successfully!"),
    onError: (error) => toast.error(error.response?.data?.message || "Failed to admit guest"),
  });
